...
from .distributed import LocalEvaluator
from .meta_grammar import ProbabilisticGrammar
...
def constrained_spell(depth: int = 3, grammar: Optional[ProbabilisticGrammar] = None) -> Expr:
    ...
    left = constrained_spell(depth - 1, grammar)
    right = constrained_spell(depth - 1, grammar)

    if grammar:
        op = grammar.sample_comb(left, right, COMBS)
    else:
        op = random.choice(COMBS)
...
class EvolutionEngine:
    """Evolutionary Programming over ALCHEM-J ASTs with curriculum learning and meta-grammar."""
    def __init__(
        self,
        fitness_fn: Callable[[Expr, int], float],
        curriculum: Optional[Callable[[int], Dict[str, Any]]] = None,
        evaluator: Any = None,
        grammar: Optional[ProbabilisticGrammar] = None
    ):
    ...
    def evolve(self, pop_size: int = 50, generations: int = 20, max_depth: int = 3, constrained: bool = False) -> Expr:
        ...
        if constrained:
            population = [constrained_spell(depth=max_depth, grammar=self.grammar) for _ in range(pop_size)]
        else:
        ...
        for gen in range(generations):
            ...
            # 2. Evaluation
            fitness_scores = self.evaluator.evaluate(population, self.fitness_fn, gen)
            ...
            # 4. Meta-Learning: Update grammar if available
            if self.grammar:
                self.grammar.update(survivors)
